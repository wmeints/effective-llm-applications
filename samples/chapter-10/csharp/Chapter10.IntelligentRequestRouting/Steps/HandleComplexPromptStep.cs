...
public class HandleComplexPromptStep(ILogger<HandleComplexPromptStep> logger): KernelProcessStep
{
    [KernelFunction]
    public async Task HandlePromptAsync(Kernel kernel, string prompt)
    {
        logger.LogInformation("Handling complex prompt: {Prompt}", prompt);
        
        var completionService = kernel.GetRequiredService<IChatCompletionService>("complexPrompts");
