...
Building an agent with a basic chat history object provides maximum flexibility but it also requires a lot of work managing observability, tool access, and the data the agent has access to.

Azure offers the Azure AI Foundry Agent Service. This agent service makes it possible to build agents inside Azure with a more managed experience than the `ChatCompletionAgent` we built before. When you create an agent in Azure it's automatically connected to the telemetry module of Azure AI foundry. You can also manage tools and data sources through the AI foundry portal. It gives you less control, but may be a good option if you're looking for a bit more productivity.

To use the Azure AI Foundry Agent Service you'll need to set up an instance of the Azure AI Foundry with a project in your subscription. 

First, open the Azure Portal on https://portal.azure.com. Click on the "Create new Resource" button to start the wizard for setting up a new resource. Search for "Azure AI Foundry" and create it. 

![The input needed to create a new hub and project](foundry-step-01.png)

The wizard asks for the name of a hub and the first project. You usually only need one hub per environment (development, test, production) which hosts multiple projects. Note that agents in Azure AI foundry share tools and data within a single project. So if you don't want an agent to see content from another agent you should create a separate project for that agent.

After providing the name for the hub and project, you need to provide information about what sort of network access you want, how the identity should be setup for the agent and how data encryption should be handled. Depending on the setup you're looking for you may want to spend some time here to set things up so you have a private network and set up a user assigned identity. I'm not going to cover these details in the book as they depend on the policies that your organization has around Azure.

![Review the settings of your foundry project and hub](foundry-step-02.png)

Once you've specified the settings you want for the AI Foundry hub and project, you get an overview of the final settings. Click the Create button to deploy the hub and project. It will take a few minutes to configure the environment. After it's completed you can open the new resource in the Azure AI Foundry Portal.

![Azure AI Foundry Portal](foundry-step-03.png)

Now that you have an environment, it's time to set up a model deployment and create an agent. First, let's create the model deployment.

Navigate to the "Models + Endpoints" module in the sidebar and deploy a GPT-4.1 model with the default settings.

![Deploying a model in Azure AI Foundry](foundry-step-04.jpeg)

After deploying the model, we can create a new agent by going to the "Agents" module. A new agent is automatically created the first time you click on the "Agents" module in the Azure AI Foundry portal. 

![The agent module in the Azure AI Foundry portal](foundry-step-05.png)

On this page, you can configure the agent. I configured the agent to use the prompt we discussed earlier in this chapter and renamed it to the FeatureFileGeneratorAgent. The changes to the agent's instructions are automatically saved.

![Agent configuration details](foundry-step-06.png)

You can test the agent through the portal by clicking "Try in playground" button next
to the agent setup in the portal.

To use the agent in Semantic Kernel, we need to add a new package to the project called
`Microsoft.SemanticKernel.Agents.AzureAI`. It's a preview package so make sure you
use the `--prerelease` flag if you add this package with `dotnet add`. 

Next, use the following code to access the agent:
...
var persistentAgent = _persistentAgentsClient.Administration.GetAgent(
    configuration["Foundry:AgentId"]);

var agent = new AzureAIAgent(persistentAgent, _persistentAgentsClient);
```

This code performs the following steps:

1. The first line configures the persistent agents class and takes an endpoint and a set of credentials. I configured it to use [the Azure CLI](https://learn.microsoft.com/en-us/cli/azure/install-azure-cli) credentials to log in to the service. You can use the command `az login` to get credentials.

2. Next, we retrieve the agent we created in the portal based on it's identifier. We pull the identifier from the configuration file.

3. Finally, we create a new `AzureAIAgent` instance with the agent definition and connect it to the Azure AI foundry project.

You can now start to chat with the agent with the following code:

```csharp
var agentThread = new AzureAIAgentThread(persistentAgentsClient);
var responseStream = agent.InvokeAsync(
    new ChatMessageContent(AuthorRole.User, "..."), agentThread);
```

We can create a new thread by initializing the `AzureAIAgentThread` class with the 
persistent agents client. Alternatively, you can get an existing thread with the following code:

```csharp
var thread = await persistentAgentsClient.Threads.GetThreadAsync("...");
```

Be aware that threads aren't tied to specific users at all. It's your job to map threads to users. It requires some extra administration in your application's database.

Notice how persistent agents don't use the Kernel and any of your Semantic Kernel tools. This is because tools are managed through the Azure Portal. You can create tools for Azure Agents with Azure Functions or by providing an MCP Server. If you want to learn how to build an MCP server in C#, I recommend looking at the MCP.NET package. There's an excellent introduction [on the Microsoft blog website](https://devblogs.microsoft.com/dotnet/mcp-server-dotnet-nuget-quickstart/).

If find it a little annoying that Microsoft decided upon this route for persistent agents as it doesn't make sense to mix Semantic Kernel with Azure AI foundry like this. However, I did want to show you that you can build agents this way so you can make an informed decision whether you should go this route.
